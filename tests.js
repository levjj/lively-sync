...
'testing', {
    ...
    testNullAndUndefined: function() {
        ...
        snapshotC.data.registry.X.a = null; //FIXME: undefined still broken
    ...
    testNewObjectAtSamePath: function() {
        var x = {id:"X", name: "x"}, y = {id:"Y", name: "y", n: 3}, z = {id:"Z", name: "z", n: 3};
        x.a = y;
        var snapshotA = this.serialize({X:x});
        x.a = z;
        var snapshotB = this.serialize({X:x});
        var expected = {"X/a": [{id: "Z", name: "z", n: 3}]};
        this.assertPatch(expected, snapshotA, snapshotB);
    },
    ...
    testNestedArray: function() {
        var x = {id:"X", name: "x", a: [["b"]]};
        var snapshotA = this.serialize({X:x});
        x.a.push(["a"]);
        var snapshotB = this.serialize({X:x});
        this.assertPatch({"X": {a: {1: [["a"]]}}}, snapshotA, snapshotB);
        x.a[1].push("r");
        var snapshotC = this.serialize({X:x});
        this.assertPatch({"X": {a: {1: {1: ["r"]}}}}, snapshotB, snapshotC);
        x.a.pop();
        var snapshotD = this.serialize({X:x});
        this.assertPatch({"X": {a: {1: [0,0]}}}, snapshotC, snapshotD);
    },
    ...
    testNestedArrayWithReferences: function() {
        ...
        x.a.push([y]);
        var snapshotB = this.serialize({X:x});
        this.assertPatch({"X": {a:{0:[[]]}}, "X/a/0/0": [{id:"Y",name:"y"}]}, snapshotA, snapshotB);
        /*x.a[1].push("r");
        var snapshotC = this.serialize({X:x});
        this.assertPatch({"X": {a: {1: {1: ["r"]}}}}, snapshotB, snapshotC);
        x.a.pop();
    ...
    testColorRectangle: function () {
        ...
        var snapshotB = this.serialize(this.table);
        var expected = {};
        expected[this.rect.id + "/shape"] = {_Fill: [Color.black.asSerializedExpression()]};
        this.assertPatch(expected, snapshotA, snapshotB);
    },
